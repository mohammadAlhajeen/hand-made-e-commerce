...
public class CompanyService extends AppUserService {
    ...
    // ### page view #####
    // ###################
    public PageViewDTO upsertDraft(PageUpdateDTO dto) throws CredentialException {
        UserDetails appUser = super.userAuthorization();
        Company company = companyRepository.findByUsername(appUser.getUsername())
                .orElseThrow(() -> new CredentialException("Company not found"));
        var page = companyPageService.upsert(company.getId(), dto);
        companyPageService.addSection(page, dto, company.getId());
        return companyPageService.toView(page);
    }

    @Transactional
    public PageViewDTO getMyPage(String slug) throws CredentialException {
        UserDetails appUser = super.userAuthorization();
        Company company = companyRepository.findByUsername(appUser.getUsername())
                .orElseThrow(() -> new CredentialException("Company not found"));
        return companyPageService.get(company.getId(), slug);
    }

    public String changeMyPageStatus(String slug, PageStatus status) throws CredentialException {
        UserDetails appUser = super.userAuthorization();
        Company company = companyRepository.findByUsername(appUser.getUsername())
                .orElseThrow(() -> new CredentialException("Company not found"));
        if (status == PageStatus.PUBLISHED) {
            companyPageService.changePageStatus(company.getId(), slug, status);
        }
        return "Page status changed to " + status.toString() + " successfully";
    }
